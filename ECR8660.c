 ...
 * @ Parameter   : ECR8660_mode : 指示页内寄存器（0x1） or 页外寄存器（0x0）
		...
		switch(Moudle_number){
			case 1:
					...
					if(ECR8660_SPI_Writ == List.readwrite) {
							ECR8660_write(1,List.mode, List.addr, List.data);
							printf("write %d 0x%08X 0x%08X\r\n",List.mode, List.addr, List.data);
					}
					else {
							ECR8660_Read(1,List.mode, List.addr, &List.data);
			...
			case 2:
					...
					if(ECR8660_SPI_Writ == List.readwrite) {
							ECR8660_write(2,List.mode, List.addr, List.data);
							printf("write %d 0x%08X 0x%08X\r\n",List.mode, List.addr, List.data);
					}
					else {
							ECR8660_Read(2,List.mode, List.addr, &List.data);
							printf("read  %d 0x%08X 0x%08X\r\n",List.mode, List.addr, List.data);
					}
				break;
		}
		...
		switch(Moudle_number){
			case 1:
					for(i = 0; List[i].mode != 0xFF; i++) {
							data = _SetECR8660_to_List(1,List[i]);
					}
				break;
			case 2:
					for(i = 0; List[i].mode != 0xFF; i++) {
							data = _SetECR8660_to_List(2,List[i]);
					}
				break;
		}
...
void ECR8660_INIT(INT8U Moudle_number)
{
		switch(Moudle_number){
			case 1:
						SetECR8660_to_List(1,PwrUp_ECR8660_EN);
				break;
			case 2:
						SetECR8660_to_List(2,PwrUp_ECR8660_EN);
				break;
		}
